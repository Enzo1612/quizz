...
function Quiz() {
  ...
  const handleAnswerSelect = (isCorrect) => {
    ...
    currentQuestion.answers.map((answer, index) => {
      if (answer.isCorrect === true) {
        document.getElementById(index).style.backgroundColor = "#84dccf";
        document.getElementById(index).style.pointerEvents = "none";
      } else {
        document.getElementById(index).style.backgroundColor = "#bf98a0";
        document.getElementById(index).style.pointerEvents = "none";
      }
    });
  };
  ...
  const nextQuestion = () => {
    ...
    for (let i = 0; i < elements.length; i++) {
      elements[i].style.backgroundColor = "#312f2f";
